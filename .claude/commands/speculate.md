...
10. **Output format.** Follow [operating-brief.md](../../framework/operating-brief.md)'s output order where it applies (header, data gaps, full calculations, recommendation, next review trigger) and close with a Glossary section: run `python -m scripts.glossary_lookup TERM1 TERM2 ...` for every jargon/technical-analysis term used and paste its output — add any term it reports `NOT FOUND` to [glossary.md](../../framework/glossary.md) first, then re-run.

...
3. **Quality Score** (run unless `--valuation-only`): assemble the raw inputs and run `python -m scripts.scoring.quality_score --input <inputs.json>` for the full Quality Score (every sub-score shown) — quality can drift (margin compression, balance sheet changes, moat erosion) just like valuation. **Paste the script's full markdown output into the session log verbatim** (satisfies "show every calculation" — the script computes it, it doesn't replace displaying it). Flag if it has dropped below 80.0 (`FAILS GATE` output, or a passing score under 80.0 — Phase 04 Quality Watch escalation) — a held position dropping below the gate is itself a signal worth surfacing, even though existing holdings aren't retroactively force-exited on quality alone.
4. **Valuation Score** (run unless `--quality-only`): run `python -m scripts.scoring.valuation_score --input <inputs.json>` (feeds the Rate Environment Gate off the 10Y Treasury input) for the full Phase 02 valuation score (every sub-score and modifier) — paste its output into the session log. If both scores were computed this run, run `python -m scripts.scoring.composite_score --set quality_score=<X> --set valuation_score=<Y>` to combine into the Composite Score (50/50) and paste that output too. If only one of the two was computed (mode flag used), do **not** compute a Composite Score — reuse the other half from the existing [holdings.md](../../portfolio/holdings.md) row only as reference context, and label the output clearly as a partial (Quality-only or Valuation-only) re-score.
5. Produce the action recommendation (against the Composite Score, when computed) and, if BUY/TRIM, run `python -m scripts.scoring.order_setup --input <inputs.json>` for the full order setup (buy price, sell target, stop loss, R/R, position size — cross-checked against the allocation cap table; flags rather than silently passing an R/R below 2:1) — paste its output into the session log. Skip the action recommendation for `--quality-only` runs (no valuation input to act on) — just report the quality-gate status.

...
- You ALWAYS write for a non-finance reader. Maintain [glossary.md](glossary.md) as the standing definitions file. Every output ends with a "Glossary" section (see OUTPUT FORMAT step 9): run `python -m scripts.glossary_lookup TERM1 TERM2 ...` for exactly the jargon terms/abbreviations/unit-shorthand that output actually used (never read the whole 700+-line file) and paste its output. If it reports a term `NOT FOUND`, add that term to glossary.md first, in the same session, then re-run the lookup and cite it. Applies everywhere the human investor reads output: chat responses, session logs, decision logs, and GitHub PR descriptions/comments.
...
OUTPUT FORMAT — ALWAYS PRODUCE IN THIS ORDER
  ...
  9. Glossary: run `python -m scripts.glossary_lookup TERM1 TERM2 ...` for every jargon term/abbreviation used above and paste its output verbatim (add new terms to glossary.md first if it reports `NOT FOUND`, then re-run)

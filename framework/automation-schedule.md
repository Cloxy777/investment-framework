...
2. **Branch-push permission:** none of the six routines need "Allow unrestricted branch pushes" anymore — all six (including Routine 2 and Routine 6) push only to default `claude/`-prefixed branches and open a PR. Routine 2 and Routine 6 also call `enable_pr_auto_merge` (squash) on their PR, and the **repo** setting **Settings → General → Pull Requests → "Allow auto-merge"** should still be checked once for `cloxy777/investment-framework` (no MCP/CLI tool can toggle this remotely) so that mechanism is live the moment this repo has CI or required reviews to gate on. **In practice, today, that checkbox doesn't matter:** this repo has neither, so every PR is immediately mergeable and `enable_pr_auto_merge` short-circuits with "already in clean status... merge directly" rather than queuing anything — confirmed empirically via a throwaway test PR (PR #63, closed without merging). Routine 2 and Routine 6 therefore fall back to merging the PR directly (squash) whenever auto-merge reports this, which is the normal case — that's what actually delivers the "no manual click" behavior the original auto-merge redesign intended. See [decisions/2026-06-22-automation-routine-auto-merge-pr.md](../decisions/2026-06-22-automation-routine-auto-merge-pr.md) for the original direct-push → auto-merge-PR change, and [decisions/2026-06-22-automation-routine-auto-merge-fallback.md](../decisions/2026-06-22-automation-routine-auto-merge-fallback.md) for this direct-merge fallback. Routines 1, 3, 4, 5 leave their PRs open for manual review/merge, as before — no auto-merge needed there since their output is a proposal, not a refresh.
...
1. Follow portfolio/sync-sop.md "Full IBKR Sync" exactly: fetch positions
   ...
   title, and enable auto-merge (squash) on it - if that reports the PR
   already clean/mergeable with nothing pending (the normal case here, since
   this repo has no CI or required reviews), merge it directly (squash)
   instead of leaving it open. Do the watchlist
...
1. Quiet run (no new posts identify a resolvable, action-worthy company
   ...
   (squash) on it - if that reports the PR already clean/mergeable with
   nothing pending, merge it directly (squash) instead - and stop. Do NOT
   open a GitHub issue and do NOT send a

...
- **`framework/`** — the rules themselves. Read these before any analysis task:
  ...
  - [speculation-module.md](framework/speculation-module.md) — v1 draft: rules for a small, separate satellite sleeve for short-horizon (days/weeks/months) speculative trades, gated by the same 80.0+ Quality Score bar as the core framework; see [decisions/2026-07-01-framework-change-speculation-module.md](decisions/2026-07-01-framework-change-speculation-module.md) for why
  - [speculation-strategies.md](framework/speculation-strategies.md) — evidence-graded menu of proven short-horizon strategies/techniques (momentum breakout, PEAD, catalyst binary event, mean-reversion bounce) and the regime classification that picks between them; feeds `/speculate`
- **`portfolio/`** — [holdings.md](portfolio/holdings.md) (current positions/weights/scores), [sync-sop.md](portfolio/sync-sop.md) (how to pull live broker data into [`snapshots/`](portfolio/snapshots/) and refresh holdings), [override-log.md](portfolio/override-log.md) (every position entered outside the framework's rules, tracked and reviewed annually), and [speculation-log.md](portfolio/speculation-log.md) (every trade entered under the speculation module, tracked and reviewed monthly)
...
- `/speculate [ticker]` — evaluate one ticker for the speculation sleeve: Quality Score gate, live price/options/news data, regime-based strategy pick, levels, and a gated IBKR order recommendation (see [speculation-module.md](framework/speculation-module.md) and [speculation-strategies.md](framework/speculation-strategies.md))
...
- `/safe-guard` — no parameters; pulls active IBKR orders + live cash balances and checks the worst-case simultaneous-fill margin exposure. Minimalistic like `/healthcheck`: silent when exposure is ≤$5,000; above that, opens/updates a `margin-safeguard` GitHub issue and sends an `‼️ IMPORTANT ‼️` Telegram alert with the full breakdown and a suggested `/update-orders` invocation (manual run, or scheduled as Routine 8 — see automation-schedule.md)
- `/update-orders [grouping]` — turns a `/safe-guard` OCA-grouping proposal (or a self-derived one) into a concrete manual checklist for reorganizing active orders in TWS/Client Portal to cap worst-case margin exposure; re-validates every order ID against live state first and never places, modifies, or cancels an order itself — no MCP tool in this repo can do that

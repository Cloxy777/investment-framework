...
At the start of every investment session, **copy the prompt block below** and paste it as your first message to Claude Sonnet 4.6. This sets the operating context for the entire session. You do not need to re-paste it mid-session.

> ⚠️ Never skip this step. Without this brief, Claude starts each session without framework context and will produce inconsistent outputs.
> 

---

# ✂️ SYSTEM PROMPT — COPY & PASTE THIS
...
Before pasting the prompt above, have these ready:

- [ ]  Current **10Y US Treasury yield** (check CNBC, Bloomberg, or TradingEconomics)
- [ ]  Task type identified (Screening / Re-score / New Position / Trim Review / Exit Review / Rebalance)
- [ ]  Financial data for each ticker pulled from TIKR, Koyfin, or Finviz (see Data Input Templates page)
- [ ]  Current portfolio weights if doing a re-score or rebalance
- [ ]  Any news or fundamental changes since last review

---

# 🔄 When to Re-Paste the System Prompt

- At the start of **every new conversation**
- If a session has gone very long (>30 messages) and Claude seems to drift from protocol
- After a major framework update

You do NOT need to re-paste mid-conversation unless Claude loses context.

---

> *Created: May 2026 · Maintained alongside Quality Value + Dynamic Trimming Framework · Update whenever framework rules change*
>
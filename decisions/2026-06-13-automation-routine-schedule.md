...
1. Create a shared cloud environment (`investment-automation`) with custom network access (`eodhd.com`, `fred.stlouisfed.org`, `www.interactivebrokers.com`), the `EODHD_API_KEY` environment variable (the real key currently only lives in the gitignored `.claude/settings.local.json`, which cloud sessions never see), and the Interactive Brokers connector enabled.

...
**Addendum 2 (same day) — self-gating cadences instead of "Nth weekday of month" cron:** while setting up Routines 3–5 (Quarterly Rate Review, Monthly Screening, Monthly Rebalance), it turned out the Routines scheduler's custom-cron support doesn't document whether combined day-of-month + day-of-week fields use AND or OR semantics — so a `30 12 1-7 * 1`-style expression for "first Monday of the month" risks over-firing (e.g. every Monday *and* every 1st–7th day, depending on implementation). Rather than rely on that, all three routines are scheduled on a plain weekly/monthly preset (every Monday / every Saturday / the 1st of each month) and self-gate as the first thing their prompt does: if today isn't the specific day they're meant to act on (first Monday/Saturday of the month, or a Q-start month for the rate review), the session ends immediately with no commit, PR, or issue. This costs a handful of harmless no-op runs per month/quarter in exchange for correctness regardless of the scheduler's actual cron semantics. `automation-schedule.md`'s cadence rows and prompts for Routines 3–5 were updated accordingly. The Google Calendar `.ics` is unaffected — Google Calendar's `RRULE`/`BYSETPOS` correctly expresses "first Monday/Saturday of the month" on its own, independent of the routine scheduler.
